...
if !exists("g:DoxygenToolkit_moduleTag")
    let g:DoxygenToolkit_moduleTag = ""
endif
if !exists("g:DoxygenToolkit_pythonStyle")
    let g:DoxygenToolkit_pythonStyle = 0
endif
...
function! <SID>DoxygenLicenseFunc()
  ...
  if(s:CheckFileType() == "cpp")
    exec "normal o".strpart( s:startCommentBlock, 1 ).substitute( "Author: ".g:DoxygenToolkit_authorName, "\<enter>", "\<enter>".s:interCommentBlock, "g")
    exec "normal o".strpart( s:startCommentBlock, 1 ).substitute("Brief: ", "\<enter>", "\<enter>".s:interCommentBlock, "g")
  endif
  ...
  if(s:CheckFileType() == "python" || g:DoxygenToolkit_pythonStyle == 1)
    exec "normal o"."\"\"\"\<enter>"
    exec "normal O"."\<enter>Brief: This is "
    mark d
    exec "normal o"."\<enter>Author: ".g:DoxygenToolkit_authorName." <".g:DoxygenToolkit_authorTag.">\<enter>"
    let l:date = strftime("%Y-%m-%d")
    exec "normal o"."Date: ".l:date."\<enter>"
  endif
  exec "normal `dA"
...
function! <SID>DoxygenCommentFunc()
  ...
  elseif(s:CheckFileType() == "python" || g:DoxygenToolkit_pythonStyle == 1)
    let s:compactOneLineDoc = "no"
    exec "normal o".substitute(s:startCommentTag, "[[:blank:]]*$", "", "")
    exec "normal O".s:interCommentTag.g:DoxygenToolkit_briefTag_pre
  else
    let s:compactOneLineDoc = "no"
    let l:insertionMode = s:StartDocumentationBlock()
...
function! s:InitializeParameters()
  ...
  elseif(s:CheckFileType() == "python" || g:DoxygenToolkit_pythonStyle == 1)
    let s:startCommentBlock   = "#################################################################################\<enter>#\<enter># "
    let s:interCommentBlock   = "#"
    let s:endCommentBlock     = "################################################################################"
    let s:startCommentTag = "\"\"\"\<CR> "

...
# Get the PCIe domain, bus, device, function of the NIC
pci_dbdf=`ethtool -i $ethx | grep bus-info | cut -d' ' -f2`
if [ -z $pci_dbdf ]
then
...
# Get the IRQs that belong to the NIC
array_of_irqs=(`grep $ethx /proc/interrupts | sed "s/: .*//g" | sed "s/^ *//g"`)
